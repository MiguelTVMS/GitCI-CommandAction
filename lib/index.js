...
Commands.prototype.execute = function () {
    ...
    commands.forEach(function (element, index, array) {
        var commandLine = element.command;
        if (typeof element.args !== "undefined")
            commandLine += " " + element.args.join(" ");

        var child = execCommand(commandLine, function (error, stdout, stderr) {
